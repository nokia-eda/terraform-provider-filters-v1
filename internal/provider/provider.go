...
import (
	...
	"github.com/nokia/eda/apps/terraform-provider-filters/internal/eda/utils"
...
var _ provider.Provider = (*filtersProvider)(nil)

func New(ver string) func() provider.Provider {
	return func() provider.Provider {
		return &filtersProvider{version: ver}
	}
}

type filtersProvider struct {
	version string
}
...
func (p *filtersProvider) Schema(ctx context.Context, req provider.SchemaRequest, resp *provider.SchemaResponse) {
...
func (p *filtersProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
...
func (p *filtersProvider) Metadata(ctx context.Context, req provider.MetadataRequest, resp *provider.MetadataResponse) {
	resp.TypeName = "filters-v1"
	resp.Version = p.version
}

func (p *filtersProvider) DataSources(ctx context.Context) []func() datasource.DataSource {
	return []func() datasource.DataSource{
		NewAppGroupDataSource,
		NewControlPlaneFilterDataSource,
		NewControlPlaneFilterListDataSource,
		NewFilterDataSource,
		NewFilterListDataSource,
...
func (p *filtersProvider) Resources(ctx context.Context) []func() resource.Resource {
	return []func() resource.Resource{
		NewControlPlaneFilterResource,
		NewFilterResource,
